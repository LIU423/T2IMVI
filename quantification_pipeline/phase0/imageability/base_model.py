"""
Base model interface for imageability evaluation.
...
class BaseImageabilityModel(ABC):
    """
    Abstract base class for imageability evaluation models.
    
    To add a new model:
    1. Create a new file (e.g., new_model.py)
    2. Inherit from BaseImageabilityModel
    ...
    def get_yes_no_logits(self, prompt: str) -> LogitResult:
        ...
        Args:
            prompt: The complete prompt including idiom.
    ...
    def format_prompt(self, idiom: str, system_prompt: str) -> str:
        """
        Format the idiom into the model's expected prompt format.
